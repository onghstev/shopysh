...
export default function PayablesPage() {
  ...
  const exportCsv = () => {
    const csvRows = [
      ['Vendor', 'Total Outstanding', 'Current (0-30)', '31-60 Days', '61-90 Days', '90+ Days'],
      ...rows.map(r => [
        r.vendorName,
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'ap-aging.csv'; a.click();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
            <Truck className="w-5 h-5" style={{ color: 'hsl(168 84% 26%)' }} />
            Accounts Payable
          </h1>
          <p className="text-muted-foreground text-sm mt-0.5">
            {data?.account ? `${data.account.code} — ${data.account.name} · ` : ''}AP aging report
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { label: 'Total AP', value: totals.totalOutstanding, color: 'hsl(168 84% 26%)' },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider py-3 px-4">Vendor</th>
