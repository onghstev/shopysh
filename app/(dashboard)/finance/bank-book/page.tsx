...
export default function BankBookPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(result);
      // Set default account if not selected yet
      if (!selectedAccountId && result.account?.id) setSelectedAccountId(result.account.id);
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `bank-book-${from}-${to}.csv`; a.click();
    URL.revokeObjectURL(url);
  };

  const lines: any[] = data?.lines ?? [];
  const totals = data?.totals ?? { totalReceipts: 0, totalPayments: 0, closingBalance: 0 };
  const allAccounts: any[] = data?.allAccounts ?? [];
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
            <Building2 className="w-5 h-5" style={{ color: 'hsl(168 84% 26%)' }} />
            Bank Book
          </h1>
          <p className="text-muted-foreground text-sm mt-0.5">
            {data?.account ? `${data.account.code} — ${data.account.name}` : 'Running ledger for bank account'}
          </p>
        </div>
        ...
        {allAccounts.length > 1 && (
          <div className="space-y-1">
            ...
            <select
              ...
              className="h-9 rounded-xl border border-input bg-background px-3 text-sm min-w-48"
            >
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-16 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">No bank transactions in this period</p>
                  </td>
                </tr>
              ) : (
