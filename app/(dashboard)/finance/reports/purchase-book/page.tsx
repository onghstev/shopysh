...
import { Loader2, Printer, ShoppingCart, ChevronLeft } from 'lucide-react';
...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  PURCHASE_INVOICE: { label: 'Purchase Invoice', color: 'bg-orange-100 text-orange-700 border-orange-200' },
  PURCHASE_PAYMENT: { label: 'Payment',          color: 'bg-blue-100 text-blue-700 border-blue-200' },
  DEBIT_NOTE:       { label: 'Debit Note',        color: 'bg-violet-100 text-violet-700 border-violet-200' },
};
...
export default function PurchaseBookReportPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/finance/purchase-book?from=${from}&to=${to}&limit=200`);
  ...
  return (
    <div className="space-y-4 print:space-y-2">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold">Purchase Book Report</h1>
          </div>
        </div>
      ...
      <ReportPrintHeader title="Purchase Book Report" subtitle={`Period: ${from} to ${to}`} />
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4 print:hidden">
            {[
              ...
              { label: 'Total Debit (Expenses)', value: fmt(data.totals?.totalDebit ?? 0), color: 'text-orange-600' },
              { label: 'Total Credit (AP / Cash)', value: fmt(data.totals?.totalCredit ?? 0), color: 'text-red-600' },
            ].map(c => (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-muted/20 border-b border-border">
                    ...
                    <th className="text-right px-4 py-2.5 font-semibold text-orange-700">Debit (DR)</th>
                    <th className="text-right px-4 py-2.5 font-semibold text-red-700">Credit (CR)</th>
                ...
                <tbody>
                  {entries.map((e: any) => {
                    ...
                    return (
                      <tr key={e.id} className="border-b border-border/20 hover:bg-muted/10">
                        ...
                        <td className="px-4 py-2 text-right font-mono text-orange-700">{fmt(Number(e.totalDebit))}</td>
                        <td className="px-4 py-2 text-right font-mono text-red-700">{fmt(Number(e.totalCredit))}</td>
                  ...
                  {entries.length === 0 && (
                    <tr><td colSpan={8} className="px-4 py-8 text-center text-muted-foreground">No purchase entries in this period.</td></tr>
                  )}
                </tbody>
                {entries.length > 0 && (
                  <tfoot>
                    <tr className="bg-muted/30 border-t border-border font-semibold">
                      ...
                      <td className="px-4 py-2.5 text-right font-mono text-orange-700">{fmt(data.totals?.totalDebit ?? 0)}</td>
                      <td className="px-4 py-2.5 text-right font-mono text-red-700">{fmt(data.totals?.totalCredit ?? 0)}</td>
