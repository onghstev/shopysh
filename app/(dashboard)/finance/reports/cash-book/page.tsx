...
import { Loader2, Printer, Banknote, ChevronLeft } from 'lucide-react';
...
export default function CashBookReportPage() {
  ...
  const receipts = lines.filter(l => l.debit > 0);
  ...
  return (
    <div className="space-y-4 print:space-y-2">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold">Cash Book Report</h1>
          </div>
        </div>
      ...
      <div className="hidden print:block text-center mb-6">
        <h1 className="text-lg font-bold">Cash Book Report</h1>
        <p className="text-sm text-gray-600">Period: {from} to {to}</p>
        <p className="text-xs text-gray-500 mt-1">Account: {data?.account?.code} — {data?.account?.name}</p>
      </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-muted/50 border-b border-border">
                  ...
                  <th className="text-left px-4 py-2.5 font-semibold text-muted-foreground w-20">Type</th>
              ...
              <tbody>
                ...
                {lines.map((l, i) => (
                  <tr key={l.id} className={`border-b border-border/20 ${l.status === 'DRAFT' ? 'opacity-60' : ''} hover:bg-muted/10`}>
                    <td className="px-4 py-2 text-muted-foreground whitespace-nowrap">
                      {format(new Date(l.date), 'dd MMM yyyy')}
                    </td>
                    <td className="px-4 py-2">
                      <span className="font-medium">{l.description}</span>
                      {l.customer && <span className="text-muted-foreground ml-1">— {l.customer.name}</span>}
                    </td>
                    <td className="px-4 py-2 text-muted-foreground font-mono text-[10px]">{l.reference ?? l.entryNumber}</td>
                    <td className="px-4 py-2">
                      {l.status === 'DRAFT' && (
                        <Badge variant="outline" className="text-[9px] text-amber-600 border-amber-200">Draft</Badge>
                      )}
                    </td>
                ...
                <tr className="bg-muted/40 border-t border-border font-semibold">
                  <td colSpan={4} className="px-4 py-2.5 text-right text-xs">Period Totals</td>
