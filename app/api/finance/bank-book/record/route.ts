...
export async function POST(req: NextRequest) {
  ...
  const bankId = await findSystemAccount(tenantId, 'BANK');
  if (!bankId) {
  ...
  try {
    const entry = await createAndPostJournal({
      ...
      description,
      entryType: type === 'RECEIPT' ? 'BANK_RECEIPT' : 'BANK_WITHDRAWAL',
      createdById: userId,
      lines,
    }, userId);
