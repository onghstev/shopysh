  ...
  try {
    ...
    const bankAccounts = await prisma.glAccount.findMany({
      where: {
        tenantId,
        isActive: true,
        OR: [{ systemTag: 'BANK' }, { accountSubtype: 'bank' }],
      },
      orderBy: { code: 'asc' },
    });

    if (bankAccounts.length === 0) {
      return NextResponse.json({ error: 'No bank accounts found. Please set up your Chart of Accounts.' }, { status: 404 });
    }

    // Use specified account or default to first
    const account = accountId
      ? bankAccounts.find(a => a.id === accountId) ?? bankAccounts[0]
      : bankAccounts[0];

    const fromDate = new Date(from);
    const toDate = new Date(to + 'T23:59:59Z');

    // Prior lines for opening balance
    ...
    return NextResponse.json({
      ...
      allAccounts: bankAccounts.map(a => ({ id: a.id, code: a.code, name: a.name })),
      openingBalance,
