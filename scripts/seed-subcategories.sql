...
-- Run: cat scripts/seed-subcategories.sql | docker exec -i shopysh-db-1 psql -U shopysh -d shopysh

DO $$
DECLARE
  t_id UUID := 'a6f03f3f-a95c-4e56-8624-fc665e3187a1';
  p_id UUID;
BEGIN

  -- ── Food & Beverages ──────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Food & Beverages' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Fashion & Clothing ────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Fashion & Clothing' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Health & Wellness ─────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Health & Wellness' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Beauty & Personal Care' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Phones & Tablets ─────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Phones & Tablets' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Electronics & Gadgets ─────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Electronics & Gadgets' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Computers & Accessories' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Home & Living ─────────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Home & Living' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Cleaning & Household Supplies' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Agriculture & Farm Produce' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Baby & Kids ───────────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Baby & Kids' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Sports & Fitness ──────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Sports & Fitness' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Automotive & Vehicle Parts' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Building & Construction' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Jewelry & Accessories ─────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Jewelry & Accessories' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Books & Stationery ────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Books & Stationery' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Arts & Crafts ─────────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Arts & Crafts' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Traditional & Cultural Items' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Industrial & Business Supplies' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
  ...
  -- ── Gifts & Novelties ─────────────────────────────────────────────────────
  SELECT id INTO p_id FROM public.product_categories WHERE tenant_id=t_id AND name='Gifts & Novelties' AND parent_id IS NULL;
  IF p_id IS NOT NULL THEN
    INSERT INTO public.product_categories(id,tenant_id,name,description,display_order,parent_id,is_active,created_at,updated_at)
    VALUES
